...
/** Pure transformation functions for target items. These are used mostly with offline keys, so we
  * can return the metadata as it would have been transformed if the operation had succeeded, but we
  * also use them directly when editing custom metadata fields, just so we have a single source of
  * truth.
  *
...
object TargetItemTransformations {
  ...
  def transformTargetCustomForEdit(existingCustom: TargetCustom,
                                   targetEdit: EditTargetItem): TargetCustom =
    existingCustom.copy(
  ...
  def transformTargetCustomForProprietaryUpdate(existingCustom: TargetCustom,
                                                proprietaryJson: Json): TargetCustom = {
  ...
  def transformItemsForDelete(filename: TargetFilename)(
    allItems: Seq[TargetItem]): Try[Seq[TargetItem]] =
    Success(allItems.filter(_.filename != filename))

  def transformItemsForEdit(filename: TargetFilename, targetEdit: EditTargetItem)(
    allItems: Seq[TargetItem]): Try[Seq[TargetItem]] =
    allItems
  ...
  def transformItemsForProprietaryUpdate(filename: TargetFilename, proprietaryJson: Json)(
    allItems: Seq[TargetItem]): Try[Seq[TargetItem]] =
    allItems
      .find(_.filename == filename)
      .map { existingTarget =>
