...
trait FakeTufServerClient extends TufServerClient {
  ...
  protected val pushedDelegations =
    new ConcurrentHashMap[DelegatedRoleName, SignedPayload[TargetsRole]]()
  ...
  override def root(version: Option[Int] = None): Future[SignedPayload[RootRole]] =
    if (version.isDefined)
      ...
      val unsignedRoot = RootRole(
        Map(oldPair.pubkey.id -> oldPair.pubkey, targetsPubKey.id -> targetsPubKey),
        Map(
      ...
      val sig =
        TufCrypto.signPayload(oldPair.privkey, unsignedRoot.asJson).toClient(oldPair.pubkey.id)
  ...
  def delegations(): List[(DelegatedRoleName, SignedPayload[TargetsRole])] =
    pushedDelegations.asScala.toList

}

object FakeReposerverTufServerClient {

  def apply(keyType: KeyType): FakeReposerverTufServerClient = new FakeReposerverTufServerClient(
    keyType
  )

}

class FakeReposerverTufServerClient(val keyType: KeyType)
    extends ReposerverClient
    with FakeTufServerClient
    with DirectorClient {
  ...
  override def targets(): Future[TargetsResponse] = Future.successful {
    val sig =
      TufCrypto.signPayload(targetsPair.privkey, unsignedTargets.asJson).toClient(targetsPubKey.id)
    val signedPayload = SignedPayload(Seq(sig), unsignedTargets, unsignedTargets.asJson)
    TargetsResponse(
      signedPayload,
      Option(
        Refined.unsafeApply("095c33175e5af42691c1b41d388c8ce842c7fbb792fcc6514b5436f7f80420db")
      )
    )
  }

  override def pushTargets(targetsRole: SignedPayload[TargetsRole],
                           checksum: Option[Refined[String, ValidChecksum]]): Future[Unit] =
  ...
  override def pullDelegation(name: DelegatedRoleName): Future[SignedPayload[TargetsRole]] =
    Future.successful {
      Option(pushedDelegations.get(name))
        .getOrElse(throw new IllegalArgumentException("[test] delegation not found"))
    }
