...
object Cli extends App with VersionInfo {
  ...
  lazy val repoNameOpt: OptionParser[Config] => OptionDef[RepoName, Config] = { parser =>
    parser
      ...
      .text(
        "The name of the local repository. This repository should be a directory in your `tuf` repository. You can create the repository with the `init` command."
      )
  }
    ...
    parser
      ...
      .validate { json =>
        ...
        if (badFields.exists(json.hcursor.downField(_).succeeded))
          parser.failure(
            s"Fields ${badFields.mkString("`", ",", "`")} not allowed when specifying custom metadata"
          )
        else
          parser.success
      }
  ...
  lazy val parser = new scopt.OptionParser[Config](PROGRAM_NAME) {
    ...
    cmd("delegations")
      .children(
        ...
        cmd("sign")
          ...
          .children(
            ...
            opt[Path]("input")
              ...
              .required()
              .toConfigOptionParam(Symbol("inputPath"))
              .text("The path to the delegated Targets metadata file that you want to sign."),
            opt[Unit]("inplace")
              .abbr("e")
              .optional()
              .action { case (_, c) => c.copy(inplace = true) }
              .text(
        ...
        cmd("push")
          ...
          .children(
            repoNameOpt(this),
            opt[DelegatedRoleName]("name")
              .abbr("n")
              .required()
              .toConfigParam(Symbol("delegationName"))
              .text("The name of the delegation."),
            opt[Path]("input")
              .abbr("i")
              .required()
              .toConfigOptionParam(Symbol("inputPath"))
              .text("The path to the signed .json file with delegations.")
          )
          .text(
            "Pushes delegation metadata to the server. Requires an initialized `tuf` repository."
          ),
        cmd("pull")
          ...
          .children(
            ...
            opt[Path]("output")
              .abbr("o")
              .toConfigOptionParam(Symbol("outputPath"))
              .text("The name of the file to which you want to save the delegation.")
          )
          .text(
            "Pulls a delegated Targets metadata file from the server. Requires an initialized `tuf` repository."
          ),
        cmd("add-target")
          ...
          .children(
            opt[Path]("input")
              ...
              .required()
              .toConfigOptionParam(Symbol("inputPath"))
              .text("The path to the delegated Targets metadata file that you want to modify."),
            opt[Unit]("inplace")
              .abbr("e")
              .optional()
              .action { case (_, c) => c.copy(inplace = true) }
    ...
    cmd("move-offline")
      ...
      .text(
        "Removes online keys from OTA Connect, and updates the environment to use locally stored offline keys."
      )
      .children(
        repoNameOpt(this),
        opt[KeyName]("new-root")
          .text(
            "(Optional) The new Root key that you want to add to the `root.json` file. If not provided, you'll have to manually sign and push it with `root sign` and `root push`."
          )
          .toConfigOptionParam(Symbol("rootKey")),
    ...
    cmd("root")
      ...
      .children(
        ...
        cmd("pull")
          ...
          .children(
            opt[Unit]("force")
              .action((_, c) => c.copy(force = true))
              .text(
                "Skips validation of the remote `root.json` file against the local `root.json` file"
              )
              .hidden()
          )
        ...
        cmd("push")
          .toCommand(PushRoot)
          .text(
            "Uploads local `root.json` file to OTA Connect. If the file does not have a valid signature, it will be rejected by the server."
          ),
        cmd("get-unsigned")
        ...
        cmd("key")
          .children(
            cmd("add")
              ...
              .text(
                "Adds a specific key to the list of keys authorized to sign the root-of-trust metadata."
              ),
            cmd("remove")
              .toCommand(RemoveRootKey)
              .children(
                manyKeyNamesOpt(this).text(
              ...
              .text(
                "Removes a specific key from the list of keys authorized to sign the root-of-trust metadata."
              )
          )
        ...
        cmd("targets-key")
          .children(
            cmd("add")
              ...
              .text(
                "Adds a specific key to the list of keys authorized to sign the targets metadata."
              ),
            cmd("remove")
              ...
              .text(
                "Removes a specific key from the list of keys authorized to sign the targets metadata."
              )
          )
        ...
        cmd("sign")
          ...
          .children(
            manyKeyNamesOpt(this).optional().text("The path to the public key to use for signing.")
          )
    ...
    cmd("targets")
      .text(
        """(Only for repositories of type `reposerver`) Manages Targets metadata.
      ...
      .children(
        ...
        cmd("init")
          ...
          .children(
            opt[Int]("version")
              ...
              .required()
              .text(
                """The version of the `targets.json` file.
        ...
        cmd("add-uploaded")
          ...
          .text(
            "Adds a target that you previously uploaded to OTA Connect using the `targets upload` command."
          ),
        cmd("delete")
          ...
          .children(
            opt[TargetFilename]("filename")
              ...
              .text(
                """The exact name of the target to remove.
        ...
        cmd("sign")
          ...
          .children(
            ...
            opt[Int]("version")
              .text(
                "The version number to use for the signed metadata. Overrides the version in the unsigned `targets.json` file."
              )
              .toConfigOptionParam(Symbol("version")),
            opt[Map[KeyName, ValidSignatureType]]("signatures")
              .action { (m, c) =>
                if (m.isEmpty) c.copy(signatures = None) else c.copy(signatures = Some(m))
              }
            ...
            cmd("add")
              ...
              .children(
                opt[DelegatedRoleName]("name")
                  ...
                  .toConfigParam(Symbol("delegationName"))
                  .text("The name of the target."),
                opt[DelegatedPathPattern]("prefix")
                  .abbr("p")
                  .unbounded()
                  .minOccurs(1)
    ...
    cmd("export-credentials")
      ...
      .children(
        ...
        manyKeyNamesOpt(this)
          .text("The name of the file with your private and public keys that you want to export."),
        opt[Path]("output")
          .abbr("o")
    ...
    cmd("verify-root")
      ...
      .hidden()
      .children(repoNameOpt(this), opt[Path]('i', "input").toConfigOptionParam(Symbol("inputPath")))
    ...
    cmd("import-client-cert")
      ...
      .children(
        ...
        opt[Path]('c', "client-cert")
          .text(
            "The path to the valid PKCS#12 file that can be used to authenticate to a remote tuf repository."
          )
          .required()
          .toConfigOptionParam(Symbol("inputPath")),
        opt[Path]('s', "server-cert")
          .text(
            "The path to the valid PKCS# file that can be used as a trust store to validate a remote `tuf` repository certificate."
          )
  ...
  def validateRepoTypeCliArguments(repoPath: Eval[Path],
                                   config: Config): Either[String, TufServerType] = {
    val repoType =
    ...
    validateRepoTypeCliArguments(repoPath, config) match {
      case Right(Director) =>
        Eval.later {
          ...
          lazy val delegationsServer = Future.failed(
            CliArgumentsException(
              s"$repoPath is a Director repository and does not support delegations"
            )
          )

          CommandHandler.handle(tufRepo, repoServer, delegationsServer, userKeyStorage, config)
        }
      case Right(RepoServer) =>
        Eval.later {
  ...
  def execute(config: Config): Unit = {
    ...
    try {
      ...
      System.exit(
        0
      ) // Needed because AsyncHttpClient might still be using idle threads for it's connection pool
    } catch {
  ...
  def setLoggingLevel(level: Level): Unit = {
    ...
    val sttpLogger: Logger =
      LoggerFactory.getLogger("sttp.client.logging.slf4j").asInstanceOf[Logger]
