...
object ClientCodecs {
  ...
  implicit val roleTypeKeyEncoder: KeyEncoder[RoleType] =
    KeyEncoder.encodeKeyString.contramap[RoleType](_.toString.toLowerCase)

  implicit val roleTypeKeyDecoder: KeyDecoder[RoleType] =
    KeyDecoder.decodeKeyString.map[RoleType](s => RoleType.withName(s.toUpperCase))
  ...
  implicit val targetCustomEncoder: Encoder[TargetCustom] = Encoder.instance { targetCustom =>
    ...
    val withCliUploaded =
      if (targetCustom.cliUploaded.isDefined)
  ...
  implicit val delegationClientTargetItemEncoder: Encoder[DelegationClientTargetItem] =
    deriveEncoder

  implicit val delegationClientTargetItemDecoder: Decoder[DelegationClientTargetItem] =
    deriveDecoder
  ...
  implicit val delegatedRoleNameEncoder: Encoder[DelegatedRoleName] =
    ValidatedString.validatedStringEncoder

  implicit val delegatedRoleNameDecoder: Decoder[DelegatedRoleName] =
    ValidatedString.validatedStringDecoder

  implicit val delegatedRoleNameKeyEncoder
    : io.circe.KeyEncoder[com.advancedtelematic.libtuf.data.ClientDataType.DelegatedRoleName] =
    new KeyEncoder[DelegatedRoleName] {
      override def apply(roleName: DelegatedRoleName): String = roleName.value
    }

  implicit val delegatedRoleNameKeyDecoder
    : io.circe.KeyDecoder[com.advancedtelematic.libtuf.data.ClientDataType.DelegatedRoleName] =
    new KeyDecoder[DelegatedRoleName] {

      override def apply(key: String): Option[DelegatedRoleName] =
        DelegatedRoleName.delegatedRoleNameValidation(key).toOption

    }

  implicit val delegationFriendlyNameEncoder: Encoder[DelegationFriendlyName] =
    ValidatedString.validatedStringEncoder

  implicit val delegationFriendlyNameDecoder: Decoder[DelegationFriendlyName] =
    ValidatedString.validatedStringDecoder

  implicit val delegatedPathPatternEncoder: Encoder[DelegatedPathPattern] =
    ValidatedString.validatedStringEncoder

  implicit val delegatedPathPatternDecoder: Decoder[DelegatedPathPattern] =
    ValidatedString.validatedStringDecoder
  ...
  implicit val targetsRoleEncoder: Encoder[TargetsRole] =
    deriveEncoder[TargetsRole].encodeRoleType.mapJson(_.dropNullValues)

  implicit val targetsRoleDecoder: Decoder[TargetsRole] =
    deriveDecoder[TargetsRole].validateRoleType

  implicit val targetsRoleCodec: Codec[TargetsRole] =
    Codec.from(targetsRoleDecoder, targetsRoleEncoder)

  implicit val offlineUpdatesRoleEncoder: Encoder[OfflineUpdatesRole] =
    deriveEncoder[OfflineUpdatesRole].encodeRoleType

  implicit val offlineUpdatesRoleDecoder: Decoder[OfflineUpdatesRole] =
    deriveDecoder[OfflineUpdatesRole].validateRoleType

  implicit val offlineUpdatesRoleCodec: Codec[OfflineUpdatesRole] =
    Codec.from(offlineUpdatesRoleDecoder, offlineUpdatesRoleEncoder)

  implicit val offlineSnapshotRoleEncoder: Encoder[OfflineSnapshotRole] =
    deriveEncoder[OfflineSnapshotRole].encodeRoleType

  implicit val offlineSnapshotRoleDecoder: Decoder[OfflineSnapshotRole] =
    deriveDecoder[OfflineSnapshotRole].validateRoleType

  implicit val offlineSnapshotRoleCodec: Codec[OfflineSnapshotRole] =
  ...
  implicit val remoteSessionsRoleCodec: Codec[RemoteSessionsRole] =
    Codec.from(remoteSessionsRoleDecoder, remoteSessionssRoleEncoder)

  implicit val snapshotRoleEncoder: Encoder[SnapshotRole] =
    deriveEncoder[SnapshotRole].encodeRoleType

  implicit val snapshotRoleDecoder: Decoder[SnapshotRole] =
    deriveDecoder[SnapshotRole].validateRoleType

  implicit val snapshotRoleCodec: Codec[SnapshotRole] =
    Codec.from(snapshotRoleDecoder, snapshotRoleEncoder)

  implicit val timestampRoleEncoder: Encoder[TimestampRole] =
    deriveEncoder[TimestampRole].encodeRoleType

  implicit val timestampRoleDecoder: Decoder[TimestampRole] =
    deriveDecoder[TimestampRole].validateRoleType

  implicit val timestampRoleCodec: Codec[TimestampRole] =
    Codec.from(timestampRoleDecoder, timestampRoleEncoder)

  implicit private class EncodeRoleTypeOp[T](encoder: Encoder[T])(implicit tr: TufRole[T]) {

    def encodeRoleType: Encoder[T] =
      encoder.mapJson(_.mapObject(_.add("_type", tr.decoderDiscriminator.asJson)))

  }

  implicit private class ValidateRoleOp[T](decoder: Decoder[T])(implicit tr: TufRole[T]) {

    def validateRoleType: Decoder[T] = decoder.validate(
      { c =>
